...
class Graph(object):
    ...
    def __init__(self, graph_dict={}):
        """ initialize graph object: a dicitonary"""
    ...
    def clean_graph(self, actor, target):
        ...
        try:
            ...
            print("Can't remove " + actor + "or " + target + "from graph.")
            # for later addign to error log file
...
class Window(object):
    """ Window holds the 60 second frame for evaluation. Manages adding and popping
        elements from both the window list, and the Graph dictionary.
        input order is = TIMESTAMP, ACTOR, TARGET."""
    
    def __init__(self, window=[(datetime.datetime(1, 1, 1, 1, 1, 1))], nodes=[{}]):
        """ initiate window list, with dummy entry. This saves a lot of work later."""
        self.window = window
        self.nodes = nodes

    def insert(self, timestamp, actor, target, graph):
        # NOTE: bisect algorithm is fairly slow at scale, so this 
        # require change to inserting from the end of the list retroactively
        # checking. which, while is max O(N), in practice due to 60s window
        # will be shorter than log2(N)        
...
def main():
    ...
    # Streamable data would be read from API
    # we will not mimic that here, instead will call transactions one-by-one in the 
